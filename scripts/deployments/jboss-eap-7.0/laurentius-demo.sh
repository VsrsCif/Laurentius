...
if [ "x$JBOSS_HOME" = "x" ]; then
	echo "JBOSS_HOME folder not defined! Check parameters!"
	quit;
fi
...
if [ "x$LAU_HOME" = "x" ]; then
	LAU_HOME="$JBOSS_HOME/standalone/data/laurentius-home";
fi
...
$JBOSS_HOME/bin/standalone.sh $LAU_OPTS -b $LISTEN_MASK
