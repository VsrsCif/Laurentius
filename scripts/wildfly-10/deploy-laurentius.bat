...
copy "%LAU_BUNDLE%\modules\Laurentius-msh-xsd-1.0.jar" "%WILDFLY_HOME%\modules\org\sed\main\"
copy "%LAU_BUNDLE%\modules\Laurentius-wsdl-1.0.jar" "%WILDFLY_HOME%\modules\org\sed\main\"
copy "%LAU_BUNDLE%\modules\Laurentius-commons-1.0.jar" "%WILDFLY_HOME%\modules\org\sed\main\"
rem  copy module descriptor
copy "%LAU_BUNDLE%\modules\org.sed.module.xml" "%WILDFLY_HOME%\modules\org\sed\main\module.xml"

rem  deploy commons ejbs
copy "%LAU_BUNDLE%\deployments\Laurentius-dao.jar"  "%WILDFLY_HOME%\standalone\deployments\"
copy "%LAU_BUNDLE%\deployments\sed-basic-tasks.jar"  "%WILDFLY_HOME%\standalone\deployments\"
rem  deploy modules 
...
copy "%LAU_BUNDLE%\deployments\Laurentius-webgui.war"  "%WILDFLY_HOME%\standalone\deployments\"
...
if "%INIT%" == "true" (
	...
	copy "%LAU_BUNDLE%\widlfly-10\config\sed-roles.properties" "%WILDFLY_HOME%\standalone\configuration\"
	copy "%LAU_BUNDLE%\widlfly-10\config\sed-users.properties" "%WILDFLY_HOME%\standalone\configuration\"
	copy "%LAU_BUNDLE%\widlfly-10\config\standalone-ebms.xml" "%WILDFLY_HOME%\standalone\configuration\"

	rem  copy start scripts
	echo copy start scripts "%WILDFLY_HOME%\bin\
	copy "%LAU_BUNDLE%\widlfly-10\start-sed.bat" "%WILDFLY_HOME%\bin\"

	echo copy init laurentius.home to "%LAU_HOME%\laurentius.home".
