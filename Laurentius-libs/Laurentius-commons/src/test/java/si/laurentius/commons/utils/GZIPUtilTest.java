...
public class GZIPUtilTest {
  ...
  public void testGZIPNotCompressed()
    ...
    assertTrue("Compressed string 'abcd' should be bigger than source", source.length() <
        target.length());
    assertEquals("Source file and decompressed file should have equal size", source.length(),
        decomTarget.length());
    assertEquals("Source file and decompressed file should have equal hash value", mhu.getMD5Hash(
        source), mhu.getMD5Hash(decomTarget));
  ...
  public void testDetectCorruptedData()
    ...
    File badSource = tuUtils.createFile( DatatypeConverter.parseHexBinary(dmgdaga));
    File decomTarget = tuUtils.createEmptyFile();
    GZIPUtil gzipInstalce = new GZIPUtil();
