...
public enum MimeValue {
  ...
  MIME_XLTX("xltx",
          ...
          true),
  MIME_SOAP("soap", "application/soap+xml", true),
  /**
   *
   */
  ...
  MimeValue(String suffix, String mimeType, boolean defForMime) {
  ...
  /**
   * Method returns suffix for mimetype if no mimetype is found
   * MIME_BIN.getSuffix is returned-
   *
   * @param strMimeType
   * @return suffix
   */
  public static String getSuffixBYMimeType(String strMimeType) {
    ...
    for (MimeValue vm : values()) {
      if (vm.getMimeType() != null && vm.getMimeType().equalsIgnoreCase(
              strMimeType)) {
        res = vm.getSuffix();
        if (vm.getDefForMime()) {
          break;
  ...
  /**
   * Method returns mimetype according filename suffix. If filename is null or
   * mimetype is not found "MIME_BIN" mimetype is returned.
   *
   * @param strFileName
  ...
  public static String getMimeTypeByFileName(String strFileName) {
    ...
    return strFileName == null || strFileName.isEmpty() ? MIME_BIN.getMimeType()
            : getMimeTypeBySuffix(strFileName.substring(strFileName.lastIndexOf(
                    '.') + 1));
  }
