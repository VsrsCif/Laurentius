...
public class CRLVerifierTest {
  ...
  public void test_A_GetCrlData()
    ...
    for (String[] tst : LST_DATA_CRT) {
      ...
      try (InputStream inStrm = CRLVerifierTest.class.getResourceAsStream("/certs/sample/" + tst[0])) {
        CertificateFactory cf = CertificateFactory.getInstance("X.509");
        cert = (X509Certificate) cf.generateCertificate(inStrm);
        testCrlData(cert, tst[1],
            tst[2], tst[3]);
      }
    }
  }
  ...
  private void testCrlData(X509Certificate scrl, String issuer, String http, String ldap)
      throws CertificateParsingException, IOException {
    SEDCertCRL crl = CRLVerifier.getCrlData(scrl);
    Assert.assertNotNull(crl);
    Assert.assertEquals(issuer, crl.getIssuerDN());
    Assert.assertEquals(http, crl.getHttp());
    Assert.assertEquals(ldap, crl.getLdap());
  }
